 ...
 */
public class CarParkApp3D extends Application {
    ...
    private static final int H_PAD  = 70;   // left/right entry-road padding
    ...
    private static final String C_SPACE_3D = "#334155";   // slate-700  (3D-effect)
    ...
    // ── Isometric projection styling ────────────────────────────────────────
    private static final double ISO_X_SHEAR = 0.5;
    private static final double ISO_Y_SCALE = 0.6;
    private static final double ISO_ANGLE   = -10.0;
    private static final double SLOT_DEPTH  = 6;
    ...
    private int    nCols;
    private double lotW, lotH, laneCY;
    ...
    private final List<Polygon>   spaceRects3D = new ArrayList<>();
    private Group[]    parkedCars;
    private Timeline[] activeAnims;
    ...
    private boolean laneOccupied = false;
    private final Deque<Runnable> laneQueue = new ArrayDeque<>();
    ...
    public void start(Stage stage) {
        ...
        startUpdateLoop();
        controller.start();
        controller.setProductionRateMs(1500);
        controller.setConsumptionRateMs(4000);
    ...
    private HBox buildHeader() {
        ...
        resetBtn.setOnAction(e -> {
            ...
            if (prodRateSlider  != null) prodRateSlider.setValue(1500);
            if (consRateSlider  != null) consRateSlider.setValue(3000);
            if (procTimeSlider  != null) procTimeSlider.setValue(SimulationController.DEFAULT_PROC_TIME_MS);
        });
    ...
    private Pane buildLotPane() {
        ...
        // Apply isometric transformations for 3D effect
        Shear shear = new Shear();
        shear.setX(ISO_X_SHEAR);
        lotPane.getTransforms().add(shear);
        lotPane.setScaleY(ISO_Y_SCALE);
        lotPane.setRotate(ISO_ANGLE);
    ...
    private void drawLotBackground(Pane p, double w, double h) {
        ...
        // Curb strips — subtle lighter edge along top and bottom
        ...
        // Lane edge lines (thin white rules)
        ...
        p.getChildren().addAll(laneEdgeTop, laneEdgeBot);

        // Entry / exit approach roads
        Rectangle entryRoad = new Rectangle(0, V_PAD + SH, H_PAD, LANE_H);
        entryRoad.setFill(Color.web(C_LANE, 0.60));
        Rectangle exitRoad  = new Rectangle(w - H_PAD, V_PAD + SH, H_PAD, LANE_H);
        exitRoad.setFill(Color.web(C_LANE, 0.60));
        p.getChildren().addAll(entryRoad, exitRoad);

        // Dashed yellow centre line
        double dashY = V_PAD + SH + LANE_H / 2.0 - 2;
        for (double x = H_PAD; x < w - H_PAD; x += 24) {
            Rectangle d = new Rectangle(x, dashY, 14, 4);
            d.setFill(Color.web("#FBBF24", 0.68));
            d.setArcWidth(2); d.setArcHeight(2);
            p.getChildren().add(d);
        }

        // ENTER label
        Label inLbl = new Label("ENTER \u2192");
        inLbl.setFont(Font.font("Segoe UI", FontWeight.BOLD, 10));
        inLbl.setTextFill(Color.web("#4ADE80"));
        inLbl.setLayoutX(4); inLbl.setLayoutY(laneCY - 9);
        ...
        outLbl.setLayoutX(w - 52); outLbl.setLayoutY(laneCY - 9);
    ...
    private void drawParkingSpaces(Pane p) {
        ...
        for (int i = 0; i < capacity; i++) {
            ...
            Polygon r3d = new Polygon(
                sx, sy + SH,
                sx + SW, sy + SH,
                sx + SW, sy + SH + SLOT_DEPTH,
                sx, sy + SH + SLOT_DEPTH
            );
            r3d.setFill(Color.web(C_SPACE_3D));
            spaceRects3D.add(r3d);
            p.getChildren().add(r3d);


            // Front marking line (bottom of top spaces, top of bottom spaces)
    ...
    private VBox buildControlsCard() {
        ...
        prodRateSlider = mkSlider(100,  3000, 1500);
        consRateSlider = mkSlider(100,  3000, 3000);
        procTimeSlider = mkSlider(50,   2000, SimulationController.DEFAULT_PROC_TIME_MS);

        capacityValLbl = ctrlValueLabel(String.valueOf(SimulationController.DEFAULT_CAPACITY));
        prodRateValLbl = ctrlValueLabel("1500 ms");
        consRateValLbl = ctrlValueLabel("3000 ms");
        procTimeValLbl = ctrlValueLabel(SimulationController.DEFAULT_PROC_TIME_MS + " ms");
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // Lane gate — only ONE car in the driving lane at a time (unchanged)
    // ─────────────────────────────────────────────────────────────────────────
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // Arrival animation:  entrance-left → lane column → parking space (unchanged)
    // ─────────────────────────────────────────────────────────────────────────
    private void animateArrive(int idx) {
        Runnable task = () -> doAnimateArrive(idx);
        if (laneOccupied) laneQueue.offer(task);
        else { laneOccupied = true; task.run(); }
    }

    private void doAnimateArrive(int idx) {
        if (idx >= capacity || lotPane == null) { releaseLane(); return; }
        ...
        boolean top    = (idx % 2 == 0);
        int     col    = idx / 2;
        double  colCX  = H_PAD + col * (SW + GAP) + SW / 2.0;
        double  spaceCY = top ? V_PAD + SH / 2.0
                              : V_PAD + SH + LANE_H + SH / 2.0;

        Group car = makeCar(CAR_COLORS[idx % CAR_COLORS.length]);
        car.setLayoutX(-CAR_H / 2.0);
        car.setLayoutY(laneCY);
        car.setRotate(90);
        lotPane.getChildren().add(car);
        parkedCars[idx] = car;

        double driveDur = 600.0 + col * 350.0;

        Timeline tl = new Timeline(
            new KeyFrame(Duration.ZERO,
                new KeyValue(car.layoutXProperty(), -CAR_H / 2.0),
                new KeyValue(car.layoutYProperty(), laneCY),
                new KeyValue(car.rotateProperty(),  90)),

            // Phase 1 — drive east to column; release lane so next car can follow
            new KeyFrame(Duration.millis(driveDur),
                ae -> releaseLane(),
                new KeyValue(car.layoutXProperty(), colCX,  Interpolator.EASE_BOTH),
                new KeyValue(car.layoutYProperty(), laneCY),
                new KeyValue(car.rotateProperty(),  90)),

            // Phase 2 — slow turn to face space
            new KeyFrame(Duration.millis(driveDur + 400),
                new KeyValue(car.rotateProperty(), top ? 0 : 180, Interpolator.EASE_BOTH)),

            // Phase 3 — creep into parking space
            new KeyFrame(Duration.millis(driveDur + 1000),
                new KeyValue(car.layoutXProperty(), colCX,   Interpolator.EASE_BOTH),
                new KeyValue(car.layoutYProperty(), spaceCY, Interpolator.EASE_BOTH))
        );
        ...
        tl.setOnFinished(e -> {
            ...
            if (idx < spaceRects.size()) {
                ...
                spaceRects3D.get(idx).setFill(Color.web(C_SPACE_O).darker());
            }
            visualOccupied++;
            visualTotalIn++;
        });
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // Departure animation:  parking space → lane → exit-right (unchanged)
    // ─────────────────────────────────────────────────────────────────────────
    ...
    private void doAnimateLeave(int idx) {
        ...
        if (slotParked[idx]) {
            ...
            if (idx < spaceRects.size()) {
                ...
                spaceRects3D.get(idx).setFill(Color.web(C_SPACE_3D));
            }
            if (visualOccupied > 0) visualOccupied--;
        }

        if (car == null) { releaseLane(); return; }

        double curX    = car.getLayoutX();
        double curY    = car.getLayoutY();
        int    col     = idx / 2;
        double colCX   = H_PAD + col * (SW + GAP) + SW / 2.0;
        double exitX   = lotW + CAR_H / 2.0;
        double exitDur = 600.0 + (nCols - 1 - col) * 300.0;

        Timeline tl = new Timeline(
            new KeyFrame(Duration.ZERO,
                new KeyValue(car.layoutXProperty(), curX),
                new KeyValue(car.layoutYProperty(), curY),
                new KeyValue(car.rotateProperty(),  car.getRotate())),

            // Phase 1 — reverse back to lane centre
            new KeyFrame(Duration.millis(700),
                new KeyValue(car.layoutXProperty(), colCX,  Interpolator.EASE_BOTH),
                new KeyValue(car.layoutYProperty(), laneCY, Interpolator.EASE_BOTH)),

            // Phase 2 — turn to face east
            new KeyFrame(Duration.millis(1100),
                new KeyValue(car.rotateProperty(), 90, Interpolator.EASE_BOTH)),

            // Phase 3 — drive east to exit; release lane after car is clear
            new KeyFrame(Duration.millis(1100 + exitDur),
                new KeyValue(car.layoutXProperty(), exitX,  Interpolator.EASE_BOTH),
                new KeyValue(car.layoutYProperty(), laneCY))
        );
    ...
    private Group makeCar(Color color) {
        // Ground shadow
        Ellipse groundShadow = new Ellipse(0, CAR_H/2.0 + 3, CAR_W/2.0 + 2, 5);
        groundShadow.setFill(Color.web("#000000", 0.22));

        // Isometric body
        double bh = 8; // body height
        double rh = 9; // roof height
        double ch = bh + rh; // car height
        Polygon body = new Polygon(
            -CAR_W/2.0, -CAR_H/2.0,
            CAR_W/2.0, -CAR_H/2.0,
            CAR_W/2.0, CAR_H/2.0 - bh,
            -CAR_W/2.0, CAR_H/2.0 - bh
        );
        body.setFill(color);

        Polygon body3D = new Polygon(
            -CAR_W/2.0, CAR_H/2.0 - bh,
            CAR_W/2.0, CAR_H/2.0 - bh,
            CAR_W/2.0, CAR_H/2.0,
            -CAR_W/2.0, CAR_H/2.0
        );
        body3D.setFill(color.darker());

        // Isometric roof
        double ro = 3; // roof overhang
        Polygon roof = new Polygon(
            -CAR_W/2.0+ro, -CAR_H/2.0+5,
            CAR_W/2.0-ro, -CAR_H/2.0+5,
            CAR_W/2.0-ro, CAR_H/2.0 - bh - rh,
            -CAR_W/2.0+ro, CAR_H/2.0 - bh - rh
        );
        roof.setFill(color.brighter());

        Polygon roof3D = new Polygon(
            -CAR_W/2.0+ro, CAR_H/2.0 - bh - rh,
            CAR_W/2.0-ro, CAR_H/2.0 - bh - rh,
            CAR_W/2.0-ro, CAR_H/2.0 - bh,
            -CAR_W/2.0+ro, CAR_H/2.0 - bh
        );
        roof3D.setFill(color);

        // Windshield
        Polygon windshield = new Polygon(
            -CAR_W/2.0+ro+2, -CAR_H/2.0+7,
            CAR_W/2.0-ro-2, -CAR_H/2.0+7,
            CAR_W/2.0-ro-2, -CAR_H/2.0+18,
            -CAR_W/2.0+ro+2, -CAR_H/2.0+18
        );
        windshield.setFill(Color.web("#BAE6FD", 0.82));

        return new Group(groundShadow, body, body3D, roof, roof3D, windshield);
    }
    ...
    private void clearLot() {
        ...
        spaceRects3D.forEach(p -> p.setFill(Color.web(C_SPACE_3D)));
